cc.Class({
        ...
        if (window.wx != undefined) {
            window.wx.onMessage(data => {
                ...
                if (data.messageType == 0) {//移除排行榜
                    ...
                    this.fetchFriendData(data.MAIN_MENU);
                } else if (data.messageType == 3) {//提交得分
                    this.submitScore(data.MAIN_MENU, data.score);
                } else if (data.messageType == 4) {//获取好友排行榜横向排列展示模式
                    this.gameOverRank(data.MAIN_MENU);
                } else if (data.messageType == 5) {//获取群排行榜
                    this.fetchGroupFriendData(data.MAIN_MENU, data.shareTicket);
                }
            });
        } else {
    ...
    submitScore(MAIN_MENU, score) { //提交得分
        if (window.wx != undefined) {
            window.wx.getUserCloudStorage({
                ...
                success: function (getres) {
                    ...
                    window.wx.setUserCloudStorage({
                        KVDataList: [{key: MAIN_MENU, value: "" + score}],
    ...
    fetchGroupFriendData(MAIN_MENU, shareTicket) {
        this.removeChild();
        this.rankingScrollView.node.active = true;
        if (window.wx != undefined) {
            wx.getUserInfo({
